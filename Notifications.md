...
_Note_: you can place 'global' configuration for notifications in a file in your home directory called ".bpipeconfig".  These will be shared by all Bpipe pipelines in any directory, but any local configuration will override global configurations.
...
```
Note that for Google Talk, you have to have a separate _sender_ account for which you enter credentials, and then a recipient account which is the account that gets notified.
...
You may wish to send out notifications explicitly as part of your pipeline, rather than relying on Bpipe to do it in response to pipeline events. This is possible via the [send](Send.md), [succeed](Succeed.md) and [fail](Fail.md) commands. See documentation on these commands for more information.
